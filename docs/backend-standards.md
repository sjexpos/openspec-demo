...
- Bucket CORS is part of the definition of done for any browser-facing presigned-upload endpoint:
  the bucket MUST permit the browser preflight (`PUT`, app origins, `content-type` header) before
...
- **Asset-Creation Convention**: `POST /<parent>/{id}/<assets>` with no request body returns `201` + `{ uploadUrl, uploadMethod, expiresAt }` (all `String`) and `Cache-Control: no-store`. The persisted asset row is a server-side side effect: **no identifier, key or lifecycle state is exposed**, which is what keeps confirmation server-side and the key scheme free to change. The controller maps the port-issued `BlobUploadTarget` directly and never touches the asset entity (stays `open-in-view`-proof by construction). No `Location` header (it would expose the withheld identifier). This is the template for all asset tables (`brand_images` first).
- **Upstream Blob-Store Failure (`502`)**: a `BlobStorageException` escaping an upload-target orchestration is mapped by a dedicated `@ExceptionHandler(BlobStorageException.class)` to `502 Bad Gateway`, with a single `ErrorResponse` error entry whose `field` is `"general"` and whose `message` is the fixed string `"Blob storage is currently unavailable"` — **never** `ex.getMessage()`, which embeds blob keys and signature-adjacent material. `502` (upstream dependency failure), not `500` (our bug) nor `503` (no retry-after known).
- **Presigned-URL Handling**: presigned URLs may be returned in a response body but are never logged at any level, never persisted, never placed in exception messages or metric tags, never used in an OpenAPI example (placeholder only), and never cached (`Cache-Control: no-store`). Wrong-method access to an asset collection URI is reported as `405 Method Not Allowed` via a dedicated `@ExceptionHandler(HttpRequestMethodNotSupportedException.class)` (the shared `Exception → 500` catch-all would otherwise convert it to `500`).
...
- **Feature Branches**: Develop features in separate branches, adding descriptive prefix "feat/, fix/, hotfix/, chore/, docs/, refactor/, test/" to allow working in parallel and avoid conflicts or collisions
