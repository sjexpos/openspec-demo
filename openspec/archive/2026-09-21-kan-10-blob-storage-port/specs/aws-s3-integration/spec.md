...
The beans are now consumed by the first storage adapter introduced by the `blob-storage` capability. Bean construction behavior is unchanged. This requirement SHALL NOT introduce a controller, health indicator, SQS consumer, bucket hardening, Testcontainers module, or async client.
...
- **THEN** the operations SHALL be served through the `S3Client` and `S3Presigner` beans defined by this requirement
- **AND** bean construction, region resolution, endpoint handling, and credential resolution SHALL remain unchanged
