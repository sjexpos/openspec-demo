...
public interface DispensaryApi {
  ...
  @Operation(
      summary = "Create a new dispensary",
      description = "Creates a new dispensary and returns it with the generated ID")
  @ApiResponses({
    @ApiResponse(responseCode = "201", description = "Dispensary created successfully"),
    @ApiResponse(responseCode = "400", description = "Invalid input")
  })
  DataResponse<CreateDispensaryResponse> create(
      @Valid @RequestBody CreateDispensaryRequest request);

  @GetMapping("/{id}")
  @ResponseStatus(value = HttpStatus.OK)
  @Operation(
      summary = "Get dispensary by ID",
      description = "Returns a single dispensary by their ID")
  @ApiResponses({
